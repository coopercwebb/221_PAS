 ...
 * be called by multiplying current capacity EXPANSIONFACTOR defined in stack.h.
 * To be clear resize should happen before adding the element to the stack and
 * only when the stack is completely full.
 *
 ...
 * to strictly less than a ratio of 1.0 / SHRINKRATE (where SHRINKRATE is
 * defined in stack.h) after the item is removed the Stack should be resized to
 * the larger of max_items / EXPANSIONFACTOR, or DEFAULTCAPACITY.
 *
 ...
 * @pre n >= num_items, i.e. the new size has room for all items in the
 * structure
